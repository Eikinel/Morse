...
public:
	...
	const std::vector<std::shared_ptr<Phase>>&	getPhases() const;
	const std::shared_ptr<Phase>&				getPhaseByTime(const sf::Time& time) const;
	...
	void	removeBezierCurveAt(const size_t& index);
...
private:
	...
	std::vector<std::unique_ptr<sf::Sound>>	_hit_sounds;
